...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "active_sessions_user_id_fkey", "active_sessions", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "selected_groups_user_id_fkey", "selected_groups", type_="foreignkey"
    )

    # Alter user_id columns in related tables to BIGINT
    op.alter_column(
        "active_sessions", "user_id", existing_type=sa.INTEGER(), type_=sa.BIGINT()
    )
    op.alter_column(
        "ai_accounts", "user_id", existing_type=sa.INTEGER(), type_=sa.BIGINT()
    )
    op.alter_column("groups", "user_id", existing_type=sa.INTEGER(), type_=sa.BIGINT())
    op.alter_column(
        "keywords", "user_id", existing_type=sa.INTEGER(), type_=sa.BIGINT()
    )
    op.alter_column(
        "selected_groups", "user_id", existing_type=sa.INTEGER(), type_=sa.BIGINT()
    )
    ...
    op.create_foreign_key(
        "active_sessions_user_id_fkey", "active_sessions", "users", ["user_id"], ["id"]
    )
    op.create_foreign_key(
        "ai_accounts_user_id_fkey", "ai_accounts", "users", ["user_id"], ["id"]
    )
    op.create_foreign_key("groups_user_id_fkey", "groups", "users", ["user_id"], ["id"])
    op.create_foreign_key(
        "keywords_user_id_fkey", "keywords", "users", ["user_id"], ["id"]
    )
    op.create_foreign_key(
        "selected_groups_user_id_fkey", "selected_groups", "users", ["user_id"], ["id"]
    )
    ...
    if "group_ai_accounts" in tables:
        op.drop_constraint(
            "group_ai_accounts_group_id_fkey", "group_ai_accounts", type_="foreignkey"
        )
        op.alter_column(
            "group_ai_accounts",
            "group_id",
        ...
        op.create_foreign_key(
            "group_ai_accounts_group_id_fkey",
            "group_ai_accounts",
            "groups",
            ["group_id"],
            ["id"],
...
def downgrade() -> None:
    ...
    op.alter_column(
        "active_sessions", "user_id", existing_type=sa.BIGINT(), type_=sa.INTEGER()
    )
    op.alter_column(
        "ai_accounts", "user_id", existing_type=sa.BIGINT(), type_=sa.INTEGER()
    )
    op.alter_column("groups", "user_id", existing_type=sa.BIGINT(), type_=sa.INTEGER())
    op.alter_column(
        "keywords", "user_id", existing_type=sa.BIGINT(), type_=sa.INTEGER()
    )
    op.alter_column(
        "selected_groups", "user_id", existing_type=sa.BIGINT(), type_=sa.INTEGER()
    )
