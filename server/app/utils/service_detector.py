...
class ServiceDetector:
    ...
    async def is_service_available(
        self, service_name: str, force_refresh: bool = False
    ) -> bool:
        """
        ...
        try:
            services_health = await asyncio.wait_for(
                self.validator.check_all_services(), timeout=5.0
            )
...
def requires_service(
    service_name: str, fallback_value=None, raise_on_unavailable: bool = False
):
    """
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if not is_available:
                    ...
                    logger.warning(
                        f"Service {service_name} unavailable, using fallback for {func.__name__}"
                    )
                ...
                logger.error(
                    f"Error in service-dependent function {func.__name__}: {e}"
                )
                if raise_on_unavailable:
                    raise ServiceAvailabilityError(
                        service_name, f"Error checking service: {str(e)}"
                    )
                return fallback_value
...
def optional_service(service_name: str, default_value=None):
    ...
    """
    return requires_service(
        service_name, fallback_value=default_value, raise_on_unavailable=False
    )
