...
async def get_group_ai_mappings(user_id: int):
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            mapping_stmt = (
                select(GroupAIAccount, Group)
                .join(Group, GroupAIAccount.group_id == Group.id)
                .where(and_(Group.user_id == user_id, GroupAIAccount.is_active is True))
            )
