...
class LoginRateLimiter:
    ...
    def _clean_expired_entries(self):
        ...
        for key, data in self._attempts.items():
            ...
            if (current_time - data.get("window_start", 0)) > (
                settings.LOGIN_RATE_LIMIT_LOCKOUT_MINUTES * 60
            ):
    ...
    def is_rate_limited(self, identifier: str) -> tuple[bool, Optional[str]]:
        ...
        if locked_until > current_time:
            ...
            return (
                True,
                f"Too many login attempts. Try again in {remaining_lockout} minutes.",
            )
        ...
        if data["attempts"] >= settings.LOGIN_RATE_LIMIT_MAX_ATTEMPTS:
            ...
            return (
                True,
                f"Too many login attempts. Account locked for {settings.LOGIN_RATE_LIMIT_LOCKOUT_MINUTES} minutes.",
            )
    ...
    def record_attempt(self, identifier: str, success: bool = False):
        ...
        if success:
            ...
            logger.info(
                f"Successful login for {identifier[:4]}****, resetting rate limit"
            )
            data["attempts"] = 0
            data["window_start"] = current_time
            data["locked_until"] = 0
            ...
            logger.warning(
                f"Failed login attempt for {identifier[:4]}****, attempt {data['attempts']}/{settings.LOGIN_RATE_LIMIT_MAX_ATTEMPTS}"
            )
