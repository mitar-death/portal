...
class EnvironmentValidator:
        ...
        validation_result = {
            ...
            "configuration": {
                "critical_missing": [
                    ...
                    if r.level == ConfigLevel.CRITICAL and not r.present
                ],
                "important_missing": [
                    r
                    for r in config_results
                    if r.level == ConfigLevel.IMPORTANT and not r.present
                ],
                "optional_missing": [
                    r
                    for r in config_results
                    if r.level == ConfigLevel.OPTIONAL and not r.present
                ],
            ...
            "recommendations": self._generate_recommendations(
                config_results, service_results
            ),
        }
    ...
    def validate_configuration(self) -> List[ConfigValidation]:
        ...
        for key, (level, description, suggestion) in self._config_requirements.items():
            ...
            if not is_present and hasattr(settings, key):
                ...
                if settings_value and str(settings_value) not in [
                    "",
                    "your-secret-key-here",
                    "jwt-secret-key-change-in-production",
                    "jwt-refresh-secret-key-change-in-production",
                ]:
                    is_present = True
                ...
                # but issue warnings if they're using default values
                    ...
                    key in ["SECRET_KEY", "JWT_SECRET_KEY", "JWT_REFRESH_SECRET_KEY"]
                    and settings_value
                ):
                    is_present = True
                    value = "[CONFIGURED]"
                    if str(settings_value) in [
                        "your-secret-key-here",
                        "jwt-secret-key-change-in-production",
                        "jwt-refresh-secret-key-change-in-production",
                    ]:
    ...
    async def _check_telegram_health(self) -> ServiceHealth:
        ...
        try:
            ...
            is_authorized = await asyncio.wait_for(
                client.is_user_authorized(), timeout=5.0
            )
            response_time = (time.time() - start_time) * 1000
    ...
    async def _check_google_ai_health(self) -> ServiceHealth:
        ...
        try:
            ...
            return ServiceHealth(
                ...
                available=True,
                response_time_ms=round(response_time, 2),
                details={"api_key_configured": True, "model": "gemini-pro"},
            )
    ...
    async def _check_pusher_health(self) -> ServiceHealth:
        ...
        try:
            pusher_configured = all(
                [settings.PUSHER_APP_ID, settings.PUSHER_KEY, settings.PUSHER_SECRET]
            )
    ...
    def _determine_overall_status(
        ...
        service_results: Dict[str, ServiceHealth],
    ) -> ServiceStatus:
        ...
        if service_results.get("telegram", {}).status in [
        ...
        for service_name, health in service_results.items():
            if not health.available:
                if service_name in ["database"]:  # Critical services
                    errors.append(
                        f"CRITICAL: {service_name.title()} service unavailable - {health.error}"
                    )
                else:
                    warnings.append(
                        f"{service_name.title()} service unavailable - {health.error}"
                    )
            elif health.status == ServiceStatus.DEGRADED:
                warnings.append(
                    f"{service_name.title()} service degraded - {health.error or 'Limited functionality'}"
                )
    ...
    def _generate_recommendations(
        ...
        if missing_important:
            recommendations.append(
                "Configure important settings to enable full functionality:"
            )
        ...
        if (
            telegram_health
            and telegram_health.available
            and telegram_health.status == ServiceStatus.DEGRADED
        ):
            recommendations.append(
                "Complete Telegram authorization to enable message monitoring"
            )

        redis_health = service_results.get("redis")
        if redis_health and not redis_health.available:
            recommendations.append(
                "Configure Redis for improved performance and session management"
            )

        pusher_health = service_results.get("pusher")
        if pusher_health and not pusher_health.available:
            recommendations.append(
                "Configure Pusher for enhanced real-time WebSocket features"
            )
    ...
    def _log_validation_results(self, results: Dict[str, Any]):
        ...
        if overall_status == "unavailable":
            ...
            logger.warning(
                "⚠️  Environment validation shows DEGRADED status - Some features unavailable"
            )
        else:
...
def can_start_application() -> bool:
    ...
    critical_missing = [
        r for r in config_results if r.level == ConfigLevel.CRITICAL and not r.present
    ]
