...
@safe_db_operation()
async def get_keywords_controller(
    ...
    try:
        ...
        return standardize_response(
            {"keywords": keywords_list, "count": len(keywords_list)},
            "Keywords fetched successfully",
        )
    except Exception as e:
        logger.error(f"Failed to fetch keywords: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch keywords: {str(e)}"
        ) from e


@safe_db_operation()
async def add_keywords_controller(
    request: Request, db: AsyncSession = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not isinstance(keywords, list):
            ...
            if isinstance(keywords, str):
                ...
                raise HTTPException(
                    status_code=400, detail="Keywords must be a list of strings"
                )
        ...
        return standardize_response(
            {"keywords": user_keywords.keywords, "added_count": added_count},
        ...
        raise HTTPException(status_code=500, detail=f"Failed to add keywords: {str(e)}") from e


@safe_db_operation()
async def delete_keywords_controller(
    request: Request, db: AsyncSession = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for keyword in keywords:
            if isinstance(keyword, str) and keyword.strip():
                if user_keywords.remove_keyword(keyword.strip()):
                    removed_count += 1
                    logger.info(f"Removed keyword: {keyword.strip()}")
        ...
        return standardize_response(
            {"keywords": user_keywords.keywords, "removed_count": removed_count},
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete keywords: {str(e)}"
        ) from e
