...
@safe_db_operation()
async def cleanup_ai_sessions(request: Request = None, user_id: Optional[int] = None, db: AsyncSession = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not accounts:
            return standardize_response(
                {"deleted_count": 0},
                "No AI accounts found for cleanup."
            )
        ...
        for account in accounts:
            ...
            if os.path.exists(session_file):
                try:
                    ...
                    logger.error(f"Error deleting session file for account {account.id}: {sanitize_log_data(str(e))}")
        ...
        logger.error(f"Database error in cleanup_ai_sessions: {sanitize_log_data(str(e))}")
...
@safe_db_operation()
async def logout_ai_account(request: Request, db: AsyncSession = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if os.path.exists(session_file):
            try:
                os.remove(session_file)
                logger.info(f"Deleted session file for account {account.id}: {sanitize_log_data(session_file)}")
                return standardize_response(
                    {},
        ...
        logger.error(f"Database error in logout_ai_account: {sanitize_log_data(str(e))}")
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
