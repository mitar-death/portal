...
def init_redis_with_retry(
    decode_responses=False, max_retries=3, base_delay=1.0
    ...
    for attempt in range(max_retries):
        try:
            ...
            client.ping()
            logger.info(
                f"Redis connection established successfully on attempt {attempt + 1}"
            )
            return client
...
def init_redis(decode_responses=False) -> Optional[redis.Redis]:
    ...
    if _redis_connection is not None:
        try:
            ...
            logger.warning(
                "Existing Redis connection is unhealthy, creating new connection"
            )
