...
from server.app.core.middlewares import (
    DBSessionMiddleware,
    AuthMiddleware,
...
from server.app.core.environment_validator import (
...
async def lifespan(app: FastAPI):
    ...
    async def initialize_app():
        try:
            ...
            if not validation_result["can_start"]:
                logger.error(
                    "❌ Cannot start application due to critical configuration issues"
                )
                for error in validation_result["errors"]:
                    logger.error(error)
                raise Exception(
                    "Critical configuration missing - application cannot start"
                )
            ...
            if validation_result["overall_status"] == "degraded":
                ...
                for warning in validation_result["warnings"][
                    :5
                ]:  # Limit warnings in startup
                    logger.warning(warning)
            else:
                logger.info(
                    "✅ Environment validation passed - all systems operational"
                )
            ...
            try:
                ...
                monitoring_task.add_done_callback(
                    lambda t: (
                        ...
                        if not t.exception()
                        else logger.error(f"Error starting monitoring: {t.exception()}")
                    )
                )
            ...
            try:
                ...
                health_check_task.add_done_callback(
                    lambda t: background_tasks.discard(t)
                )
                ...
                health_check_task.add_done_callback(
                    lambda t: (
                        ...
                        if not t.exception()
                            ...
                            f"Error starting health check: {t.exception()}"
                        )
                    )
                )
            except Exception as e:
    ...
    try:
        ...
        logger.warning(
            "App initialization taking longer than expected, starting anyway"
        )
    ...
    if remaining_tasks:
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(*remaining_tasks, return_exceptions=True), timeout=5
            )
        except asyncio.TimeoutError:
